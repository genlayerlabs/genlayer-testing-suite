#!/usr/bin/env python3
"""Validate GenLayer Testing Suite release versions and owning branches."""
...
_BRANCH_PATTERN = re.compile(
    r"^v(?P<major>0|[1-9][0-9]*)\." r"(?P<minor>0|[1-9][0-9]*)(?P<dev>-dev)?$"
)
...
def validate_branch_version(branch: str, value: str) -> ReleaseVersion:
    ...
    if branch_is_dev and not version.is_prerelease:
        raise ValueError(
            f"final release {version.normalized} must be cut from "
