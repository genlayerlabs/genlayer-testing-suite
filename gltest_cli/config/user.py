...
def transform_raw_to_user_config_with_defaults(raw_config: dict) -> UserConfig:
    ...
    networks: dict = raw_config.get("networks", {})
    default_network: str = networks.pop("default", DEFAULT_NETWORK)
    paths: dict = raw_config.get("paths", {})
    environment: str = raw_config.get("environment", DEFAULT_ENVIRONMENT)

    networks_config = {}
    for network_name, network_config in networks.items():
        # Only default network can be overridden
        if network_name == DEFAULT_NETWORK:
